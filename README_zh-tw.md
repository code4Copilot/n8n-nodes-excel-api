...
### 🎉 自動型態轉換 (v1.0.3)
- ✅ **智能型態偵測**：自動將字串轉換為適當的資料型態
- ✅ **數字轉換**：`"123"` → `123`，`"45.67"` → `45.67`
- ✅ **布林轉換**：`"true"` → `true`，`"false"` → `false`
- ✅ **空值轉換**：`"null"` 或空字串 → `null`
- ✅ **日期轉換**：ISO 格式日期字串自動轉換（`"2024-01-15"`）
- ✅ **保留已有型態**：數字、布林等已正確型態的值保持不變
- ✅ **適用所有操作**：Append 和 Update 操作都支援
...
```json
{
  "員工編號": "E001",      // 保持字串
  "年齡": "30",            // 自動轉為數字 30
  "薪資": "50000.50",     // 自動轉為 50000.50
  "在職": "true",          // 自動轉為布林 true
  "離職日期": "null",      // 自動轉為 null
  "入職日期": "2020-01-15" // 自動轉為日期格式
}
```
